...
DYNAMICS_EQUATION_STR = (
    "r[t+1] = clip(r[t] + u[t] + eps_r[t], r_min, r_max); "
    "theta[t+1] = (theta[t] + omega_true*dt + eps_theta[t]) mod 2*pi; "
    "z[t] = [r[t]cos(theta[t]), r[t]sin(theta[t])]"
)
...
def latent_vector_field(X, Y, omega, radial_u=0.0):
    R = np.sqrt(X**2 + Y**2)
    R_safe = np.maximum(R, 1e-6)
    U = radial_u * (X / R_safe) - omega * Y
    V = radial_u * (Y / R_safe) + omega * X
    return U, V


def compute_global_acq_range(sims, R):
    amin, amax = np.inf, -np.inf
    for strategy in ["dyn_only", "obs_only", "combined"]:
        sim = sims[strategy]
        for ov in sim["omega_var"]:
            _, _, A = map_from_policy(float(ov), strategy, R)
            amin = min(amin, float(np.min(A)))
            amax = max(amax, float(np.max(A)))
    return float(amin), float(amax)


def create_triptych_for_strategy(out_dir: Path, sims, strategy: str, X, Y, R, acq_cmap: str, acq_vmin: float, acq_vmax: float):
    T = PARAMS["T"]
    idxs = [0, T // 2, T - 1]
    ...
    vmins = [min(float(np.min(m[k])) for m in maps) for k in range(2)] + [acq_vmin]
    vmaxs = [max(float(np.max(m[k])) for m in maps) for k in range(2)] + [acq_vmax]

    fig, axes = plt.subplots(3, 3, figsize=(13.6, 12.0), constrained_layout=True)
    col_titles = ["I_dyn", "I_obs", f"A_total ({strategy})"]
    cmaps = ["magma", "viridis", acq_cmap]

    q_step = 16
    Uq, Vq = latent_vector_field(X[::q_step, ::q_step], Y[::q_step, ::q_step], PARAMS["omega_true"], radial_u=0.0)
...
def create_map_evolution_animation(out_dir: Path, sims, strategy: str, X, Y, R, acq_cmap: str, acq_vmin: float, acq_vmax: float):
    sim = sims[strategy]
    ...
    ims = []
    for i, (ax, Z0, cmap, lab) in enumerate(zip(axes, [I_d0, I_o0, A0], cmaps, labels)):
        im = ax.imshow(Z0, extent=[X.min(), X.max(), Y.min(), Y.max()], origin="lower", cmap=cmap, alpha=0.92)
        if i == 2:
            im.set_clim(acq_vmin, acq_vmax)
...
def create_policy_trajectory_figure(out_dir: Path, sims, X, Y, R, acq_cmap: str, acq_vmin: float, acq_vmax: float):
    fig, axes = plt.subplots(1, 3, figsize=(15.0, 4.9), constrained_layout=True)
    t_ref = PARAMS["T"] // 2
    q_step = 16
    Uq, Vq = latent_vector_field(X[::q_step, ::q_step], Y[::q_step, ::q_step], PARAMS["omega_true"], radial_u=0.0)

    for ax, strategy in zip(axes, ["dyn_only", "obs_only", "combined"]):
        sim = sims[strategy]
        _, _, A = map_from_policy(float(sim["omega_var"][t_ref]), strategy, R)
        im = ax.contourf(X, Y, A, levels=32, cmap=acq_cmap, vmin=acq_vmin, vmax=acq_vmax)
        ax.plot(sim["x"], sim["y"], color=POLICIES[strategy]["color"], lw=2.0)
        ax.scatter(sim["x"][0], sim["y"][0], s=24, c="white", edgecolor="k", zorder=5)
        ax.scatter(sim["x"][-1], sim["y"][-1], s=30, c="yellow", edgecolor="k", zorder=5)
        ax.quiver(
            X[::q_step, ::q_step],
            Y[::q_step, ::q_step],
            Uq,
        ...
        ax.set_title(f"{strategy} trajectory on A_total(tmid)")
        ax.set_aspect("equal")
        ax.set_xlabel("z1")
        ax.set_ylabel("z2")
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.03)

    fig.suptitle(f"Policy trajectories + latent vector field\n{DYNAMICS_EQUATION_STR}", fontsize=10)
    out = out_dir / "policy_trajectory_vectorfield.png"
    fig.savefig(out, dpi=220)
    plt.close(fig)
    return out
...
def compose_representative_panel(out_dir: Path):
    ...
    fig.suptitle(f"Representative panel — {EXPERIMENT_NAME}\n{DYNAMICS_EQUATION_STR}", fontsize=12)
...
def compute_metrics(sims, X, Y, R, triptychs, animations, policy_figure, acq_cmap, acq_vmin, acq_vmax):
    T = PARAMS["T"]
    ...
    outputs = {
        ...
        "policy_trajectory_figure": str(policy_figure.relative_to(OUT_DIR)),
        "script_used": "script_used.py",
    }
    ...
    return {
        ...
        "definitions": {
            ...
            "latent_dynamics_discrete": DYNAMICS_EQUATION_STR,
        },
        "acquisition_map_style": {
            "cmap": acq_cmap,
            "fixed_vmin": float(acq_vmin),
            "fixed_vmax": float(acq_vmax),
        },
        "fairness": {
...
def write_summary(out_dir: Path, metrics):
    ...
    lines.append(f"- latent dynamics (discrete): {metrics['definitions']['latent_dynamics_discrete']}")
    lines.append("")
    lines.append("Acquisition map style (shared across strategies/time/frames)")
    lines.append(f"- cmap: {metrics['acquisition_map_style']['cmap']}")
    lines.append(f"- fixed vmin: {metrics['acquisition_map_style']['fixed_vmin']:.6f}")
    lines.append(f"- fixed vmax: {metrics['acquisition_map_style']['fixed_vmax']:.6f}")
    lines.append("")
    ...
    lines.append(f"- policy trajectory + vector field figure: {out['policy_trajectory_figure']}")
    lines.append(f"- script used copy: {out['script_used']}")
...
def main():
    ...
    acq_cmap = "cividis"
    acq_vmin, acq_vmax = compute_global_acq_range(sims, R)

    triptychs = {}
    animations = {}
    for strategy in ["dyn_only", "obs_only", "combined"]:
        triptychs[strategy] = create_triptych_for_strategy(
            OUT_DIR, sims, strategy, X, Y, R, acq_cmap=acq_cmap, acq_vmin=acq_vmin, acq_vmax=acq_vmax
        )

    for strategy in ["dyn_only", "obs_only", "combined"]:
        gif_path, mp4_path = create_map_evolution_animation(
            OUT_DIR, sims, strategy, X, Y, R, acq_cmap=acq_cmap, acq_vmin=acq_vmin, acq_vmax=acq_vmax
        )
        animations[strategy] = {"gif": gif_path, "mp4": mp4_path}

    policy_figure = create_policy_trajectory_figure(OUT_DIR, sims, X, Y, R, acq_cmap, acq_vmin, acq_vmax)
    compose_representative_panel(OUT_DIR)

    shutil.copy2(Path(__file__), OUT_DIR / "script_used.py")

    metrics = compute_metrics(sims, X, Y, R, triptychs, animations, policy_figure, acq_cmap, acq_vmin, acq_vmax)
