...
# Current TBME group overview export
...
def _asset_overview_dir(group_name: str) -> Path:
    return _asset_RESULTS_DIR / group_name / "overview"


def _asset_overview_figures_dir(group_name: str) -> Path:
    return _asset_overview_dir(group_name) / "figures"
...
def _asset_write_tex(path: Path, rows: list[dict[str, object]]) -> None:
    lines = [
...
def _asset_plot_r2_threshold_stacked_bars(
    ...
    *,
    ...
    out_path = _asset_overview_figures_dir(group_name) / output_name
...
def _asset_plot_schedule_threshold_pareto() -> Path | None:
    ...
    out_path = (
        _asset_overview_figures_dir("exp03_schedule")
...
def group_overview_main(argv: list[str] | None = None) -> int:
    ...
    for group_name in groups:
        rows, written = _asset_export_group(group_name, GROUPS[group_name])
