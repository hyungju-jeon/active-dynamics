...
POLICY_COLORS = {
    ...
    "active_planning_adaptive_u20_r20_h40": "#B44E5A",
    "active_planning_adaptive_state_fixed_update_u20_r20_h40": "#8B6F47",
    "active_planning_adaptive_state_u20_r20_h40": "#C27A2C",
...
def _asset_plot_sample_efficiency(output_path: Path) -> Path:
    sources = [
        _ExperimentSuiteSource(
            "exp02_hard_damped_pendulum",
            "Hard pendulum",
            _suite_dir("exp02_hard", "exp02_hard_damped_pendulum"),
        ),
        _ExperimentSuiteSource(
            "exp02_hard_asymmetric_basin",
            "Hard basin",
            _suite_dir("exp02_hard", "exp02_hard_asymmetric_basin"),
        ),
    ]
    ...
    if plt_module is None:
        raise RuntimeError("Matplotlib is unavailable")
    fig, axes = plt_module.subplots(2, 2, figsize=(7.25, 4.35))
    for idx, source in enumerate(sources):
        ...
        ax.axhline(0.90, color=_experiment_C_NEUTRAL_LIGHT, linestyle="--", linewidth=0.75)
        ax.set_title(f"{chr(65 + idx)}. {source.label}: recovery", pad=3.0)
        ...
        ax = axes[1, idx]
        y = np.arange(len(_ASSET_BENCHMARK_POLICIES), dtype=np.float64)
        steps_to_threshold = []
        for policy_id in _ASSET_BENCHMARK_POLICIES:
            step = _experiment_r2_threshold_step(source.suite_dir, policy_id, threshold=0.90)
            steps_to_threshold.append(np.nan if step is None else float(step))
        finite_steps = [float(step) for step in steps_to_threshold if np.isfinite(step)]
        missing_y = [float(y[i]) for i, step in enumerate(steps_to_threshold) if not np.isfinite(step)]
        y_cap = max(2000.0, max(finite_steps) if finite_steps else 0.0)
        ax.barh(
            y,
            [0.0 if not np.isfinite(step) else step for step in steps_to_threshold],
            color=[_policy_color(policy_id) for policy_id in _ASSET_BENCHMARK_POLICIES],
            edgecolor=_experiment_C_STROKE,
            linewidth=0.45,
        )
        if missing_y:
            ax.scatter(
                [y_cap * 1.03 for _ in missing_y],
                missing_y,
                marker="x",
                s=17,
                color=_experiment_C_STROKE,
        ...
        ax.set_yticks(y)
        ax.set_yticklabels(
            [_asset_policy_label(policy_id) for policy_id in _ASSET_BENCHMARK_POLICIES],
            fontsize=6.3,
        )
        ax.set_xlim(0.0, y_cap * 1.12)
        ax.invert_yaxis()
        ax.set_title(f"{chr(67 + idx)}. Steps to R2 >= 0.90", pad=3.0)
    ...
    fig.legend(
        ...
        loc="upper center",
        bbox_to_anchor=(0.5, 1.01),
        ncol=len(_ASSET_BENCHMARK_POLICIES),
        fontsize=6.4,
        columnspacing=0.9,
        handlelength=1.5,
    )
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.94), w_pad=0.85, h_pad=0.95)
    return save_figure(fig, output_path, plt_module=plt_module)
...
def _asset_plot_constraints(output_path: Path) -> Path:
    ...
    if plt_module is None:
        raise RuntimeError("Matplotlib is unavailable")
    fig, axes = plt_module.subplots(1, 2, figsize=(7.25, 2.95))
    x_all = np.arange(len(sources), dtype=np.float64)
    x_bottleneck = np.arange(len(sources) - 1, dtype=np.float64)
    offsets = np.linspace(-0.28, 0.28, len(_ASSET_BENCHMARK_POLICIES))
    for idx, policy_id in enumerate(_ASSET_BENCHMARK_POLICIES):
        ...
        for source in sources:
            ...
            final_r2.append(np.nan if row["trajectory_r2_mean"] is None else float(row["trajectory_r2_mean"]))
            final_sem.append(float(row["trajectory_r2_sem"]))
            if source.label == "Nominal":
                continue
            ...
            if value is None or baseline_r2 is None:
                ...
                delta_sem.append(math.sqrt(float(row["trajectory_r2_sem"]) ** 2 + float(baseline_sem) ** 2))
        axes[0].errorbar(
            x_all + offsets[idx],
            final_r2,
            yerr=final_sem,
    ...
    axes[0].set_xticklabels([source.label for source in sources], rotation=22, ha="right")
    axes[0].set_ylabel("Final predictive R2")
    axes[0].set_ylim(-0.05, 1.05)
    axes[0].set_title("A. Endpoint fidelity under constraints")
    axes[0].legend(loc="lower left", fontsize=6.2, ncol=1)
    axes[1].axhline(0.0, color=_experiment_C_STROKE, linewidth=0.7)
    axes[1].set_xticks(x_bottleneck)
    axes[1].set_xticklabels([source.label for source in sources[1:]], rotation=22, ha="right")
    axes[1].set_ylabel("R2 change vs nominal")
    axes[1].set_title("B. Degradation from nominal")
    for ax in axes:
        _style_experiment_axis(ax)
...
def _asset_plot_mechanism(output_path: Path) -> Path:
    ...
    finite_points = np.concatenate([arr[np.isfinite(arr).all(axis=1), :2] for arr in zoom_points if arr.size])
    ...
    ax = axes[0, 1]
    event_specs = [
        ("adaptive_replan_reason", "parameter_update", "parameter replan", _policy_color(policy_id)),
        ("adaptive_replan_reason", "state_tracking_error", "state-error replan", _policy_color("active_myopic")),
        ("parameter_update_reason", "max_interval", "interval update", _policy_color("ensemble")),
        ("parameter_update_reason", "block_eig", "block-EIG update", _policy_color("active_planning_u20_r20_h40")),
    ]
    ...
    for field, value, _label, _color in event_specs:
        event_steps.append(
            [
                ...
                if (step := _safe_float(row.get("step"))) is not None and str(row.get(field, "")) == value
            ]
        )
    ...
    for label, _color in mismatch_specs:
        ...
        state_replans.append(
            sum(str(row.get("adaptive_replan_reason", "")) == "state_tracking_error" for row in rows)
        )
...
def _asset_plot_objective_ablation(output_path: Path) -> Path:
    ...
    if plt_module is None:
        raise RuntimeError("Matplotlib is unavailable")
    fig, axes = plt_module.subplots(2, 2, figsize=(7.25, 4.95), squeeze=False)
    letters = "ABCD"
    ...
    for source_idx, source in enumerate(sources):
        rows = [row for row in metric_rows if row["experiment"] == source.exp_id]
        row_by_policy = {str(row["policy_id"]): row for row in rows}
        ax_bar = axes[source_idx, 0]
        ax_curve = axes[source_idx, 1]
        y = [
            np.nan
            if row_by_policy[policy_id]["trajectory_r2_mean"] is None
            else float(row_by_policy[policy_id]["trajectory_r2_mean"])
            for policy_id in _experiment_OBJECTIVE_POLICIES
        ]
        yerr = [
            0.0
            if row_by_policy[policy_id]["trajectory_r2_sem"] is None
            else float(row_by_policy[policy_id]["trajectory_r2_sem"])
            for policy_id in _experiment_OBJECTIVE_POLICIES
        ]
        ax_bar.bar(
            x,
            y,
            yerr=yerr,
            color=[_policy_color(policy_id) for policy_id in _experiment_OBJECTIVE_POLICIES],
            edgecolor=_experiment_C_STROKE,
            linewidth=0.45,
            capsize=2.0,
            error_kw={"elinewidth": 0.55, "ecolor": _experiment_C_STROKE, "capthick": 0.55},
        )
        ax_bar.set_xticks(x)
        ax_bar.set_xticklabels(x_labels, rotation=35, ha="right", fontsize=6.4)
        ax_bar.set_ylabel("Final predictive R2")
        ax_bar.set_ylim(-0.05, 1.05)
        ax_bar.set_title(f"{letters[2 * source_idx]}. {source.label}: final")
...
def assets_main(argv: list[str] | None = None) -> int:
    ...
    asset_specs = [
        (
            "tbme_fig01_sample_efficiency.pdf",
            {"exp01_base", "exp02_hard"},
            _asset_plot_sample_efficiency,
        ),
        (
            "tbme_fig02_constraints.pdf",
            {"exp01_base", "exp06_bottleneck"},
            _asset_plot_constraints,
        ),
        (
            "tbme_fig03_mechanism_cadence_mismatch.pdf",
            {"exp02_hard", "exp07_mismatch_stress", "exp08_parameter_mismatch_stress"},
            _asset_plot_mechanism,
        ),
        (
            "tbme_fig04_objective_ablation.pdf",
            {"exp05_ablation"},
