...
FIGURE_COPIES = [
    (
        ...
        "parameter_error_over_steps.pdf",
        "exp1_duffing_parameter_error_over_steps.pdf",
    ),
    (
        "tbme_exp1_duffing_policy_sota",
        "parameter_error_over_cpu_time.pdf",
        "exp1_duffing_parameter_error_over_cpu_time.pdf",
    ),
    (
        "tbme_exp1_duffing_policy_sota",
        "parameter_covariance_trace_over_steps.pdf",
        "exp1_duffing_parameter_covariance_trace_over_steps.pdf",
    ),
    (
        "tbme_exp1_duffing_policy_sota",
        "trajectory_r2_over_steps.pdf",
        "exp1_duffing_trajectory_r2_over_steps.pdf",
    ),
    (
        "tbme_exp1_duffing_challenge_sota",
        "parameter_error_over_steps.pdf",
        "exp1_hard_duffing_parameter_error_over_steps.pdf",
    ),
    (
        "tbme_exp1_duffing_challenge_sota",
        "parameter_error_over_cpu_time.pdf",
        "exp1_hard_duffing_parameter_error_over_cpu_time.pdf",
    ),
    (
        "tbme_exp1_duffing_challenge_sota",
        "parameter_covariance_trace_over_steps.pdf",
        "exp1_hard_duffing_parameter_covariance_trace_over_steps.pdf",
    ),
    (
        "tbme_exp1_duffing_challenge_sota",
        "trajectory_r2_over_steps.pdf",
        "exp1_hard_duffing_trajectory_r2_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_sota",
        "parameter_error_over_steps.pdf",
        "exp2_duffing_parameter_error_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_sota",
        "parameter_error_over_cpu_time.pdf",
        "exp2_duffing_parameter_error_over_cpu_time.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_sota",
        "parameter_covariance_trace_over_steps.pdf",
        "exp2_duffing_parameter_covariance_trace_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_sota",
        "trajectory_r2_over_steps.pdf",
        "exp2_duffing_trajectory_r2_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_parameter_sota",
        "parameter_error_over_steps.pdf",
        "exp2_duffing_parameter_mismatch_parameter_error_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_parameter_sota",
        "parameter_error_over_cpu_time.pdf",
        "exp2_duffing_parameter_mismatch_parameter_error_over_cpu_time.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_parameter_sota",
        "parameter_covariance_trace_over_steps.pdf",
        "exp2_duffing_parameter_mismatch_parameter_covariance_trace_over_steps.pdf",
    ),
    (
        "tbme_exp2_robustness_duffing_parameter_sota",
        "trajectory_r2_over_steps.pdf",
        "exp2_duffing_parameter_mismatch_trajectory_r2_over_steps.pdf",
    ),
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export TBME summary figures/tables into manuscript-ready assets."
    )
...
def _find_suite_summary_dir(results_root: Path, suite_id: str) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(
            f"Could not find summary/metrics.csv for suite {suite_id} under {results_root}"
        )
    return candidates[0]
...
def _policy_stats(summary_dir: Path) -> dict[str, dict[str, float]]:
    ...
    for policy_id, rows in grouped.items():
        ...
        final_r2 = (
            float(traj_policy_rows[-1]["trajectory_r2_mean"]) if traj_policy_rows else math.nan
        )
        if traj_policy_rows:
            final_sem = float(traj_policy_rows[-1].get("value_sem", math.nan))
            final_n = float(traj_policy_rows[-1].get("n_points", math.nan))
            final_r2_std = (
                final_sem * math.sqrt(final_n)
                if np.isfinite(final_sem) and np.isfinite(final_n)
                else math.nan
            )
        else:
...
def _best_policy(
    stats: dict[str, dict[str, float]], allowed: set[str] | None = None
) -> tuple[str, dict[str, float]]:
    candidates = [
        (policy_id, vals)
        for policy_id, vals in stats.items()
        if allowed is None or policy_id in allowed
    ]
    if not candidates:
...
def _make_hard_duffing_exploration_overlay(results_root: Path, out_path: Path) -> None:
    ...
    low = preset.get("state_low")
    high = preset.get("state_high")
    if low is not None and high is not None:
        x_range = float(max(max(abs(float(v)) for v in low), max(abs(float(v)) for v in high)))
    else:
    ...
    for seed_dir in sorted(planning_root.glob("seed_*")):
        ...
        mean_firing = float(
            metadata.get("mean_firing_rate_target", preset.get("mean_firing_rate_target", 12.0))
        )
        max_firing = float(
            metadata.get("max_firing_rate_target", preset.get("max_firing_rate_target", 60.0))
        )
        ...
        def _trajectory_stats(run_dir: Path) -> tuple[np.ndarray, float]:
            ...
            ix = np.where(
                np.abs(axis[ix] - traj[:, 0]) < np.abs(axis[ix - 1] - traj[:, 0]), ix, ix - 1
            )
            iy = np.clip(np.searchsorted(axis, traj[:, 1]), 1, axis.shape[0] - 1)
            iy = np.where(
                np.abs(axis[iy] - traj[:, 1]) < np.abs(axis[iy - 1] - traj[:, 1]), iy, iy - 1
            )
    ...
    if not candidates:
        raise FileNotFoundError(
            "Could not find matching active_planning and baseline_prbs hard-Duffing runs"
        )
    ...
    panel_specs = [
        (
            "active_planning",
            np.asarray(best["planning_traj"], dtype=np.float32),
            "#d62728",
            float(best["planning_mean"]),
        ),
        (
            "baseline_prbs",
            np.asarray(best["prbs_traj"], dtype=np.float32),
            "#111827",
            float(best["prbs_mean"]),
        ),
    ]
    im = None
    for ax, (policy_id, traj, color, mean_logdet) in zip(axes, panel_specs):
        ...
        ax.plot(
            traj[:, 0], traj[:, 1], color="white", linewidth=3.0, alpha=0.95, solid_capstyle="round"
        )
        ax.plot(
            traj[:, 0], traj[:, 1], color=color, linewidth=1.6, alpha=0.98, solid_capstyle="round"
        )
        ax.scatter(
            traj[0, 0],
            traj[0, 1],
...
def _make_hard_duffing_phase_portrait_overlay(results_root: Path, out_path: Path) -> None:
    ...
    if not all(path.exists() for path in run_dirs.values()):
        raise FileNotFoundError(
            "Could not find seed 20 hard-Duffing trajectories for the main-text overlay"
        )
    metadata = json.loads(
        (run_dirs["active_planning"] / "run_metadata.json").read_text(encoding="utf-8")
    )
    ...
    for policy_id, color, label in policy_specs:
        ...
        ax.plot(
            traj[:, 0], traj[:, 1], color="white", linewidth=3.0, alpha=0.92, solid_capstyle="round"
        )
        ax.plot(
            traj[:, 0],
            traj[:, 1],
...
def _make_hard_duffing_schedule_sweep(track_root: Path, out_path: Path) -> None:
    ...
    for policy_id in order:
        ...
        if final_error is None:
            raise KeyError(
                "Expected embedding_error_final or parameter_error_final in run metadata"
            )
        rows[policy_id] = {
    ...
    final_mean = np.asarray(
        [rows[policy_id]["final_mean"] for policy_id in order], dtype=np.float32
    )
    runtime_mean = np.asarray(
        [rows[policy_id]["runtime_mean"] for policy_id in order], dtype=np.float32
    )
    ...
    legend_items = [
        ...
        Line2D(
            [0], [0], marker="o", color="w", markerfacecolor="#6b7280", markersize=6, label="PRBS"
        ),
    ]
    ax.legend(
...
def _make_exp1_table(results_root: Path, out_path: Path) -> None:
    ...
    lines = [
        ...
        "\\hline",
        "System & Planning err. & Planning $R^2$ & Myopic err. & Myopic $R^2$ & FLEX err. & FLEX $R^2$ & RHC err. & RHC $R^2$ & Best passive err. & Best passive $R^2$ "
        + r"\\",
        "\\hline",
    ]
...
def _make_exp2_table(results_root: Path, out_path: Path) -> None:
    ...
    lines = [
        ...
        "\\hline",
        "Mismatch regime & Planning err. & Planning $R^2$ & Next-best err. & Next-best $R^2$ & Best passive err. & Best passive $R^2$ & Runtime (planning sec) "
        + r"\\",
        "\\hline",
    ]
...
def export_assets(results_root: Path, figure_dir: Path, tex_dir: Path) -> None:
    ...
    schedule_main_track = _find_suite_track_root(
        results_root, "tbme_exp1_duffing_schedule_maintext"
    )
    ...
    _write_pgfplot_schedule_pareto(
        schedule_dir, tex_dir / "tbme_exp1_hard_duffing_schedule_pareto_plot.tex"
    )
