...
_CATALOGS: ExperimentCatalogBundle = load_catalog_bundle(
    env_catalog_paths=[SHARED_CATALOG_DIR / "experiment_env.yaml", DEFAULT_ENV_CATALOG_PATH],
    model_catalog_paths=[SHARED_CATALOG_DIR / "experiment_model.yaml", DEFAULT_MODEL_CATALOG_PATH],
...
def describe_catalogs() -> dict[str, list[str] | str]:
    return {
        "environment": [str(SHARED_CATALOG_DIR / "experiment_env.yaml"), str(DEFAULT_ENV_CATALOG_PATH)],
        "model": [str(SHARED_CATALOG_DIR / "experiment_model.yaml"), str(DEFAULT_MODEL_CATALOG_PATH)],
