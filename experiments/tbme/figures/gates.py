"""Compound tri-gate and three-gate diagnostic figure family.

Comparison, exemplary-trajectory, parameter-basis vector-field, and
observation-geometry panels for the CompoundTriGate / ThreeGateDiagnostic
systems. Unlike the suite-based families, these read run directories under
explicit result roots and write into a caller-provided output directory.
"""
...
from actdyn.visualize import compute_vector_field

from ...experiment_io import load_json, reconstruct_loglinear_rate_model
    ...
    *,
    ...
    for root in result_roots:
        for metadata_path in sorted(Path(root).glob("**/run_metadata.json")):
            ...
            if policy_id not in COMPOUND_POLICY_ORDER:
                continue
            records.append(
                RunRecord(
                    policy_id=policy_id,
...
def compound_summary_rows(
    ...
    *,
    ...
    for policy_id in COMPOUND_POLICY_ORDER:
        ...
        for record in policy_records:
            error = safe_float(record.metadata.get("embedding_error_final"))
            if error is not None:
                errors.append(error)
            r2_rows = read_trace_csv(record.run_dir / "trajectory_r2_trace.csv")
            if r2_rows:
                value = safe_float(r2_rows[-1].get("trajectory_r2"))
        ...
        rows.append(
            {
                "policy_id": policy_id,
                "label": COMPOUND_POLICY_LABELS[policy_id],
...
def compound_paired_summary_rows(
    records: Sequence[RunRecord],
) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if not r2_rows:
            continue
        final_r2 = safe_float(r2_rows[-1].get("trajectory_r2"))
        final_error = safe_float(record.metadata.get("embedding_error_final"))
...
def reach_hold_selector_occupancy(
    *,
    rest_center: float,
    target_center: float,
    gate_centers: tuple[float, float, float],
    ...
    return selector_gate_occupancy(
        trace,
        gate_centers=gate_centers,
...
def generate_compound_tri_gate_figures(
    ...
    *,
    ...
    paired_summary_rows = compound_paired_summary_rows(records)
    ...
    if rest_center is not None and rest_cutoff is not None:
        transit_occupancy = reach_hold_selector_occupancy(
    ...
    plt_module = load_plotting(summary_path, apply_style=theme.apply_style, path_is_file=True)
    if plt_module is None:
        raise RuntimeError("Matplotlib is unavailable")
    fig, axes = plt_module.subplots(2, 2, figsize=(10.0, 6.3))
    for policy_id in COMPOUND_POLICY_ORDER:
        color = COMPOUND_POLICY_COLORS[policy_id]
    ...
    axes[0, 1].axhline(0.8, color=theme.NEUTRAL_LIGHT, linestyle="--", linewidth=0.8)
    axes[0, 1].set_ylim(*r2_ylim)

    x = np.arange(len(summary_rows))
    colors = [COMPOUND_POLICY_COLORS[str(row["policy_id"])] for row in summary_rows]
    ...
    for ax, policy_id in zip(axes.ravel(), COMPOUND_POLICY_ORDER, strict=True):
        ...
        ax.plot(step, selector, color=COMPOUND_POLICY_COLORS[policy_id], linewidth=0.7)
        ax.set_title(COMPOUND_POLICY_LABELS[policy_id], fontsize=9.0)
        ax.set_ylim(
            min(
                min(gate_centers),
    ...
    plt_module = load_plotting(vector_path, apply_style=theme.apply_style, path_is_file=True)
    import torch
    ...
    plt_module = load_plotting(parameter_path, apply_style=theme.apply_style, path_is_file=True)
    fig, axes = plt_module.subplots(
        1,
        int(parameter_dim),
