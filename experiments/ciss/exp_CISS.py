...
exp_name = ["active_short", "active_long"]

meta_dynamics = MetaDynamics(hypernet_dynamics, mean_dynamics)

for exp_id in exp_name:
    base_dir = os.path.join(os.path.dirname(__file__), "../../results", "CISS_2", exp_id)
...
for k, v in e_dict.items():
    ...
    plt.plot(mean, label=k, linestyle="-", color=colorset[i])
...
plt.tight_layout()
plt.savefig(os.path.join(base_dir, "planning_embedding_error_norm.png"))
plt.show()
...
action_metric = actdyn.metrics.cost.ActionCost()
composite_metric = actdyn.metrics.CompositeMetric(
    [fisher_metric, action_metric], weights=[1.0, 0.00]
)
mpc_policy = actdyn.policy.mpc.MpcICem(
...
exp_config.results_dir = base_dir
exp_config.training.total_steps = 50000
