...
if __name__ == "__main__":
    config_path = os.path.join(os.path.dirname(__file__), "conf/obs_config.yaml")
    exp_config = ExperimentConfig.from_yaml(config_path)
    results_dir = os.path.dirname(__file__)
    base_dir = os.path.join(results_dir, "../../results", "cartpole_linear")
    ...
    exp_config.training.p_mask = 0.0
    exp_config.training.k_steps = 3
    exp_config.results_dir = os.path.join(base_dir, "residual_async_k3")
    ...
    multi_cfg.update({"p_mask": 0.0, "warmup": 0})
    loss = multi_offline_experiment.agent.model.train_model(multi_ro, **multi_cfg)
    ...
    #  Plot R2 against different methods
    fig, axs = create_subplot(r2full_m)
    for i in range(r2full_m.shape[1]):
