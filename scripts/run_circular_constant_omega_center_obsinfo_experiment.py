...
class Config:
    ...
    experiment_name: str = "circular_constant_omega_center_obsinfo_v1"
    ...
    # observation: y_t = g * exp(-||z_t||^2/(2*sigma_c^2)) + eps
    obs_center_sigma: float = 0.85

    radii_demo: tuple[float, float, float] = (0.7, 1.3, 2.0)
    demo_steps: int = 130

    steps: int = 90
    z0: tuple[float, float] = (0.9, 0.35)
    u_max: float = 0.16
...
def observation_info(z: np.ndarray, cfg: Config) -> np.ndarray:
    """
    Observation model:
      y_t = g * exp(-||z_t||^2/(2*sigma_c^2)) + v_t,   v_t ~ N(0, sigma_o^2)
    Fisher information for g:
      I_obs(z) = exp(-||z||^2 / sigma_c^2) / sigma_o^2
    """
    if z.ndim == 1:
        r2 = float(np.dot(z, z))
...
def run_policy(cfg: Config, mode: str) -> dict[str, np.ndarray]:
    ...
    for _ in range(cfg.steps):
        ...
        for u in actions:
            ...
            if mode == "dynamics_greedy":
                score = float(dynamics_info(z1, cfg)) - cfg.control_cost * float(np.dot(u, u))
            else:
                score = float(observation_info(z1, cfg)) - cfg.control_cost * float(np.dot(u, u))
            if score > best_score:
                best_score = score
                best_u = u
    ...
    return {
        ...
        "radius": np.linalg.norm(np.asarray(traj), axis=1),
    }


def radial_profile(cfg: Config, fn, r_max: float = 2.4, n: int = 120) -> tuple[np.ndarray, np.ndarray]:
    r = np.linspace(0.0, r_max, n)
    points = np.stack([r, np.zeros_like(r)], axis=-1)
    vals = fn(points, cfg)
    return r, vals
...
def _save_panel2_dynamics_info(cfg: Config, X: np.ndarray, Y: np.ndarray, Z: np.ndarray, out_path: Path) -> None:
    ...
    r, I_r = radial_profile(cfg, dynamics_info)
    ...
    ax.set_title("I_{dyn}(r) increases with radius")
...
def _save_panel3_observation_info(cfg: Config, X: np.ndarray, Y: np.ndarray, Z: np.ndarray, out_path: Path) -> None:
    ...
    r, I_r = radial_profile(cfg, observation_info)
    ...
    ax.set_title("Observation info map (center concentrated)")
    ax.set_xlabel("z_1")
    ax.set_ylabel("z_2")
    ax.set_aspect("equal")

    ax = axes[1]
    ax.plot(r, I_r, color="#377eb8", lw=2)
    ax.set_title("I_{obs}(r) decays radially")
    ax.set_xlabel("radius r")
    ax.set_ylabel("e^{-r^2/\\sigma_c^2}/\\sigma_o^2")
    ax.grid(alpha=0.25)
...
def _save_panel4_policy_timecourse(cfg: Config, dyn_pol: dict[str, np.ndarray], obs_pol: dict[str, np.ndarray], out_path: Path) -> None:
    ...
    ax = axes[1, 1]
    ax.plot(t, dyn_pol["radius"], color="#fb8072", lw=1.8, label="radius dyn-greedy")
    ax.plot(t, obs_pol["radius"], color="#80b1d3", lw=1.8, label="radius obs-greedy")
    ax.plot(t, dyn_pol["obs_info"], color="#fdb462", lw=1.5, alpha=0.9, label="I_{obs} dyn-greedy")
    ax.plot(t, obs_pol["obs_info"], color="#8dd3c7", lw=1.5, alpha=0.9, label="I_{obs} obs-greedy")
    ax.set_title("Radius + observation-info time signals")
    ax.set_xlabel("step")
    ax.set_ylabel("value")
    ax.grid(alpha=0.25)
...
def compose_panel_figure(panel_pngs: list[Path], out_png: Path, out_pdf: Path) -> None:
    ...
    for i, p in enumerate(panel_pngs):
        ...
        axes[i].imshow(img)
        axes[i].axis("off")
    fig.suptitle("Constant-omega circular dynamics with opposite spatial information gradients", fontsize=13)
...
def write_summary(cfg: Config, demo: dict[str, np.ndarray], dyn_pol: dict[str, np.ndarray], obs_pol: dict[str, np.ndarray], out_txt: Path, metrics_path: Path) -> None:
    ...
    r_grid = np.linspace(0.0, 2.3, 80)
    pts = np.stack([r_grid, np.zeros_like(r_grid)], axis=-1)
    Id = dynamics_info(pts, cfg)
    Io = observation_info(pts, cfg)
    corr_id = float(np.corrcoef(r_grid, Id)[0, 1])
    corr_io = float(np.corrcoef(r_grid, Io)[0, 1])
    radial_opposite = bool((Id[-1] > Id[0]) and (Io[-1] < Io[0]))
    ...
    metrics = {
        ...
        "dynamics_info_definition": "I_dyn(z) = dt^2 * ||z||^2 / sigma_d^2",
        "observation_model": "y = g * exp(-||z||^2/(2*sigma_c^2)) + v, v~N(0,sigma_o^2)",
        "observation_info_definition": "I_obs(z) = exp(-||z||^2 / sigma_c^2) / sigma_o^2",
        "center_vs_edge_obs_info_ratio": float(Io[0] / max(Io[-1], 1e-12)),
        "corr_radius_vs_I_dyn": corr_id,
        "corr_radius_vs_I_obs": corr_io,
        "radial_opposite_trend_verified": radial_opposite,
        ...
        "dynamics_greedy": {
            ...
            "final_radius": float(dyn_pol["radius"][-1]),
        },
        "observation_greedy": {
            "final_cum_dyn_info": float(obs_pol["cum_dyn"][-1]),
            "final_cum_obs_info": float(obs_pol["cum_obs"][-1]),
            "final_radius": float(obs_pol["radius"][-1]),
        },
    }
    metrics_path.write_text(json.dumps(metrics, indent=2), encoding="utf-8")

    lines = [
        "Circular constant-omega with center-concentrated observation information",
        "=======================================================================",
        ...
        "Observation model: y = g exp(-||z||^2/(2 sigma_c^2)) + v, v~N(0,sigma_o^2)",
        "Observation information (for g): I_obs(z) = exp(-||z||^2/sigma_c^2) / sigma_o^2",
        "",
    ...
    lines += [
        ...
        "",
        "Opposite radial trend checks:",
        f"  corr(radius, I_dyn): {corr_id:.6f} (positive expected)",
        f"  corr(radius, I_obs): {corr_io:.6f} (negative expected)",
        f"  center/edge observation-info ratio: {Io[0] / max(Io[-1], 1e-12):.4f}",
        f"  verified_opposite_trend: {radial_opposite}",
        "",
        ...
        f"  dynamics-greedy final radius: {dyn_pol['radius'][-1]:.4f}",
        f"  observation-greedy final radius: {obs_pol['radius'][-1]:.4f}",
    ]
...
def main() -> None:
    ...
    panel3 = exp_dir / "panel3_observation_info_center.png"
    panel4 = exp_dir / "panel4_policy_time_evolution.png"

    rep_png = exp_dir / "representative_circular_constant_omega_center_obsinfo.png"
    rep_pdf = exp_dir / "representative_circular_constant_omega_center_obsinfo.pdf"
    ...
    for p in [panel1, panel2, panel3, panel4, rep_png, rep_pdf, summary_txt, metrics_json]:
        print(f"[ok] wrote {p}")
