...
class ContinuousMazeEnv(gym.Env):
    ...
    def __init__(
        ...
        self.maze = np.array(
            [[0 if ch == " " else 1 for ch in line] for line in lines], dtype=np.int8
        ).T
    ...
    def step(self, action):
        ...
        x_new = (
            x
            + (wx if self.wind_field is not None else 0.0)
            + vel * np.sin(theta) * self.dt
        )
        y_new = (
            y
            + (wy if self.wind_field is not None else 0.0)
        ...
        self.state = np.array(
            [x_new, y_new, theta_new, vx_new, vy_new, omega], dtype=np.float32
        )
    ...
    def render(self):
        ...
        # goal
        pygame.draw.circle(
            self.screen, (0, 255, 0), self.goal_pos.astype(int), self.agent_radius
        )

        # boat
        x, y, theta, _, _, _ = self.state
        tip = (
            x + np.sin(theta) * self.agent_radius * 1.5,
            y + np.cos(theta) * self.agent_radius * 1.5,
        )
        pygame.draw.circle(
            self.screen, (255, 0, 0), (int(x), int(y)), self.agent_radius
        )
        pygame.draw.line(
            self.screen, (0, 0, 255), (int(x), int(y)), (int(tip[0]), int(tip[1])), 2
        )
...
if __name__ == "__main__":
    ...
    env = ContinuousMazeEnv(
        "others/generated_mazes/maze_39x19_seed1_20250618_142109.txt",
        render_mode="human",
    )
